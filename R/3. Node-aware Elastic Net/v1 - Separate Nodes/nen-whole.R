##### Elastic Net Temporal Window Cross Validation Experiment #####
require(lubridate)
require(plyr)
require(dplyr)
require(glmnet)
...
getWinEvTables <-function(data,
                          ...
                          forward=list(f.5m=5*60,f.10m=10*60,f.15m=15*60,f.20m=20*60,f.25m=25*60,f.30m=30*60), 
                          offset.back=list(),
...
load_whole_data<-function(){
  ...
  data_reduced<-data.frame("date"=data$Created_On,"node"=droplevels(data$Name),"type"=droplevels(data$Event_Type),"severity"=data$Severity)
  return(data_reduced)
}

create_node_list<-function(data){
  node_list<-vector("list",length(levels(whole_data$node)))
  names(node_list)<-levels(whole_data$node)
  for (i in 1:length(node_list)){
    node_list[[i]]<-whole_data[whole_data$node==levels(whole_data$node)[i],]
  }
  return(node_list)
}

create_win_ev_tables<-function(data){
  evTable<-data %.% tbl_df() %.% select(date,type)
...
node_event_list<-create_node_list(whole_data)
node_separated_model_list<-vector("list",length(node_event_list))
names(node_separated_model_list)<-names(node_event_list)
for (i in 1:length(node_separated_model_list)){
  winEvTable<-create_win_ev_tables(node_event_list[[i]])
  predTables<-getPredTables(winEvTable$back,winEvTable$fw)
  node_separated_model_list[[i]]<-cross_validation_for_alpha(1)
}
