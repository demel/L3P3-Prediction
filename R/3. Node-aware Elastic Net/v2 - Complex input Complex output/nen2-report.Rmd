---
title: "Node Awareness in Elastic Net v2"
author: "L3P3 Center for Open Middleware"
date: "Wednesday, October 29, 2014"
output: pdf_document
---

This document summarizes the second approach in order to expand the elastic net regularized predictor to include node awareness. This second iteration substitutes the event type field with the concatenation of the node and the event type, effectivelly allowing us to apply the same models to this new data adding a layer of node awareness. The whole process of model creation is described in the file "nen2-whole.R", found in the private L3P3 Github repository. The models are exported in a variable called "models_cc", which is a list of all the combinations of nodes and events in the system, containing each one the best created model for each event. 
...
```{r,echo=FALSE}
  ...
  models_cc[[614]]<-NULL
  split_names<-strsplit(names(models_cc),"-")
  nodes<-character()
  events<-character()
...
```{r,echo=FALSE}
  ...
  for (i in 1:length(models_cc)){
    row_index<-which(nodes==complete_nodes[[i]])
    col_index<-which(events==complete_events[[i]])
    if (is.nan(models_cc[[i]]$fscore)){model_matrix[row_index,col_index]<-0}
    else{model_matrix[row_index,col_index]<-models_cc[[i]]$fscore}
  }
```
...
```
 The percentage of created models is a 45%. A logical result, as each model has now far fewer data to train.
  
* Now, we'll study obtained fscores in the 600 correctly created models:

```{r,echo=FALSE,results='markup'}
plot1<-ggplot()+geom_histogram(aes(x=model_matrix[model_matrix>0]),fill="light blue",color="blue")+xlab("fscore")+ylab("Model amount")+ggtitle("Version 2 Model fscore")
plot1
```
The histogram shows good results for the models that actually work. A `r length(which(model_matrix>0.8999))*100/created_models`% (`r length(which(model_matrix>0.8999))` models) is at least 0.9 and a `r length(which(model_matrix>0.7999))*100/created_models`% (`r length(which(model_matrix>0.7999))` models) is at least 0.8. The average obtained fscore of the created models is `r mean(model_matrix[model_matrix>0],na.rm=TRUE)`.
