##### Elastic Net Local Data v2 (locality in observation and prediction) with 5-minute observation and prediction window#####
require(lubridate)
library(plyr)
library(dplyr)
require(glmnet)
...
load_data<-function(){
  ...
  data$type<-factor(paste(data$node,data$type,sep="-"))
...
data<-load_data()
getTables<-function(df,window_back=300,window_fw=300){
    tables_back<-list()
    tables_fw<-list()
    for (i in 1:5){
      tmp<-df[floor(((((dim(df)[1])*(i-1))/5)+1)):floor(((((dim(df)[1])*(i))/5))),]
      
      tmp_back<-ldply(unique(tmp$date),getEventsInWindow,tmp, windowSize=window_back,direction="backward", offset=0)%.% tbl_df() %.% arrange(date)
      tmp_back<-dcast(tmp_back, date ~ type, fun.aggregate= any) %.% tbl_df()
      tables_back[[i]]<-tmp_back
      
      tmp_fw<-ldply(unique(tmp$date),getEventsInWindow,tmp, windowSize=window_fw,direction="forward", offset=0)%.% tbl_df() %.% arrange(date)
...
models_cc<-vector("list",ncol(tables$fw)-1)
names(models_cc)<-names(tables$fw)[2:ncol(tables$fw)]
 for (i in 1:(ncol(tables$fw)-1)){
    ...
    print("____________")
    models_cc[[i]]<-find_best_model_trycatch(predTable)
 }
