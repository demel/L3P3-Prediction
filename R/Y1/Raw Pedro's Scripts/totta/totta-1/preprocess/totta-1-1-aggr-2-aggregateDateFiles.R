...
baseRPath <- "C:/Users/capelastegui/workspace/OFP/Totta/Totta-1/2-R/preprocess"
...
{# apply resource parser over log file groups for each (node, resource) pair. Loop parallelized for performance.
  ...
  parOptions<-list(.packages = c("plyr","gsubfn","stringr", "resourceParser"))
...
if(saveToFile){
  ...
  for (node in names(rawDataList))
  {
    ...
    if(!is.null(hd)&&dim(hd)[2]>0) {write.table(ev,evFile)} else
    {debugTable[[node]]$ev<-NA}
  }
...
#Usage
#resultList<-aggregateDataFiles(FALSE,TRUE,FALSE)