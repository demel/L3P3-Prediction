...
#utilsPath<- "C:/Users/capelastegui/workspace/OFP/utils"
 utilsPath<- "C:/Users/JM/Documents/GitHub/l3p3/Utilities"
  # folder to load data
...
events_bynode<-split(events_allnodes, events_allnodes$node)  ## WHERE DOES THIS VARIABLE COME FROM!?!?!?!?!!
...
resources_allnodes<-ldply(resources_bynode)  %.% 
  rename(replace=c(.id="node")) %.% 
  select(date,node,cpu,int,mem,hd) %.% 
  melt(c("date","node")) %.% 
  arrange(date)

# remove path from node names
resources_allnodes$node<-factor(resources_allnodes$node)
