#We now test another option for the full resource and node model: instead of adding two columns for each resource, we now
#add one column that represents the difference between immediate samples
...
create_res_df<-function(res_var,dates){
  #create column names
  columns<-vector()
  vectors<-list()
  k<-1
  for (i in 1:length(res_var)){
    print(i)
    columns<-c(columns,paste(names(res_var)[i],names(res_var[[i]]),sep="-"))
    for (j in 1:length(res_var[[i]])){
      print(j)
      res_vec<-res_var[[i]][[j]]
      res_vec$value<-c(0,diff(res_vec$value))
...
for (i in 1:length(nodes)){
  ...
  if (length(res_index)>0){
    ...
    res_tables[[j]]<-create_res_df(res_list[[res_index]],dates=tables$back$date)
    j<-j+1
  }
}
save(res_tables,file="res_tables_diff.Rdata")
