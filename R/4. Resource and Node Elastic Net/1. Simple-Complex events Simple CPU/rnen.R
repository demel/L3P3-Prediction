...
#In this first test, we add as continous variable the CPU usage of each item in the node we are predicting.
#As a first iteration, we will add the variable as a single time series.
...
for (i in 1:(ncol(tables$fw)-1)){
  ...
  if (length(res_index)>0){
    ...
    predTable<-cbind(tables$back,cpu_vector,tables$fw[,i+1])
  }
