...
#In this third test, we add as continous variable the MEM usage of each item in the node we are predicting as a single time series.
...
for (i in 1:(ncol(tables$fw)-1)){
  ...
  if (length(res_index)>0){
    mem_index<-
    mem<-res[[res_index]][[length(res_index)]][[1]]
    mem_vector<-createResVector(ev_date=tables$back$date,vec=mem)
    predTable<-cbind(tables$back,mem_vector,tables$fw[,i+1])
  }
