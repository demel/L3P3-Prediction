...
#In this second test, we add as continous variable the CPU usage of each item in the node we are predicting.
#As a second iteration, we will add the variable as two time series, its mean over time and its variation over the mean, using the
...
for (i in 1:(ncol(tables$fw)-1)){
  ...
  res_index<-which(tolower(names(res))==tolower(node_name))
  if (length(res_index)>0){
    cpu<-res[[res_index]][[1]][[1]]
    cpu_vector<-createResVector(ev_date=tables$back$date,vec=cpu)
    cpu_divided<-breakout_res(cpu_vector) ##cpu_divided[[1]]:means, cpu_divided[[2]]:variation over the mean
    predTable<-cbind(tables$back,cpu_divided[[1]],cpu_divided[[2]],tables$fw[,i+1])
  }
