##### Elastic Net Temporal Window Cross Validation Experiment applied to Totta. In this experiment, we train the models on SCF data and then
##### test them against Totta data. First, we will just create five-minute observation and prediction windows#####
require(lubridate)
require(plyr)
require(dplyr)
require(glmnet)
...
load_data<-function(){
  ...
  september<-read.csv("C:\\Users\\JM\\Documents\\experiment-data\\totta_parsed\\Septiembre_2014-events.csv",header=FALSE)
  october<-read.csv("C:\\Users\\JM\\Documents\\experiment-data\\totta_parsed\\Octubre_2014-events.csv",header=FALSE)
  november<-read.csv("C:\\Users\\JM\\Documents\\experiment-data\\totta_parsed\\Noviembre_2014-events.csv",header=FALSE)
  data<-rbind(september,october,november)
