...
critical<-logical(length(rownames(plot_df)))
crit_events<-c("96731154","68917","69481")
plot_df<-cbind(plot_df,rownames(plot_df)%in%crit_events)
names(plot_df)[length(names(plot_df))]<-"Critical"
...
plot2<-ggplot(plot_df)+geom_point(aes(x=rownames(plot_df),y=plot_df$Average,color=plot_df$Critical,size=plot_df$Critical))+xlab('Event')+ylab('F-score')+geom_errorbar(limits, width=0.25)+ theme(axis.ticks.x= element_blank(),axis.text.x = element_blank())+labs(color="Is the event critical?")+scale_color_manual(values=c("black","red"))+scale_size_manual(values=c(2,4))+guides(size=FALSE)
plot2
```
