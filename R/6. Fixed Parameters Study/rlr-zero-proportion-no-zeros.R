###RLR with no zero alteration study####
require(lubridate)
require(plyr)
require(dplyr)
require(glmnet)
...
# ones_frame<-ones_frame[sample(nrow(ones_frame)),]
  ...
  training_data<-rbind(ones_frame[1:(0.6*dim(ones_frame)[1]),],zeros_frame[1:(0.6*dim(zeros_frame)[1]),])
  validate_data<-rbind(ones_frame[((0.6*dim(ones_frame)[1])+1):(0.8*dim(ones_frame)[1]),],zeros_frame[((0.6*dim(zeros_frame)[1])+1):(0.8*dim(zeros_frame)[1]),])
  test_data    <-rbind(ones_frame[((0.8*dim(ones_frame)[1])+1):dim(ones_frame)[1],],zeros_frame[((0.8*dim(zeros_frame)[1])+1):(dim(zeros_frame)[1]),])
...
#NEW
#time_vec<-numeric()
time_no_zeros<-system.time({no_zeros_models<-cross_validation_for_alpha(target=1,zero_proportion=1)})
...
prec_df<-data.frame("1"=extract_precisions(results[[1]]),"2"=extract_precisions(results[[2]]),"3"=extract_precisions(results[[3]]),"4"=extract_precisions(results[[4]]),"5"=extract_precisions(results[[5]]),"6"=extract_precisions(results[[6]]),"7"=extract_precisions(results[[7]]),"8"=extract_precisions(results[[8]]))
rec_df<-data.frame("1"=extract_recalls(results[[1]]),"2"=extract_recalls(results[[2]]),"3"=extract_recalls(results[[3]]),"4"=extract_recalls(results[[4]]),"5"=extract_recalls(results[[5]]),"6"=extract_recalls(results[[6]]),"7"=extract_recalls(results[[7]]),"8"=extract_recalls(results[[8]]))                                                                                                                                   
fsc_df<-data.frame("1"=extract_fscores(results[[1]]),"2"=extract_fscores(results[[2]]),"3"=extract_fscores(results[[3]]),"4"=extract_fscores(results[[4]]),"5"=extract_fscores(results[[5]]),"6"=extract_fscores(results[[6]]),"7"=extract_fscores(results[[7]]),"8"=extract_fscores(results[[8]]))

prec<-list()
prec[[1]]<-prec_df
prec[[2]]<-c(mean(prec_df[,1],na.rm=TRUE),mean(prec_df[,2],na.rm=TRUE),mean(prec_df[,3],na.rm=TRUE),mean(prec_df[,4],na.rm=TRUE),mean(prec_df[,5],na.rm=TRUE),mean(prec_df[,6],na.rm=TRUE),mean(prec_df[,7],na.rm=TRUE),mean(prec_df[,8],na.rm=TRUE))
tmp<-apply(prec_df,1,function(x) {which(x==max(x))})
prec[[3]]<-c(find_appearances_list(tmp,value=1),find_appearances_list(tmp,value=2),find_appearances_list(tmp,value=3),find_appearances_list(tmp,value=4),find_appearances_list(tmp,value=5),find_appearances_list(tmp,value=6),find_appearances_list(tmp,value=7),find_appearances_list(tmp,value=8))
names(prec)<-c("df","means","maximum_appearance")

rec<-list()
rec[[1]]<-rec_df
rec[[2]]<-c(mean(rec_df[,1],na.rm=TRUE),mean(rec_df[,2],na.rm=TRUE),mean(rec_df[,3],na.rm=TRUE),mean(rec_df[,4],na.rm=TRUE),mean(rec_df[,5],na.rm=TRUE),mean(rec_df[,6],na.rm=TRUE),mean(rec_df[,7],na.rm=TRUE),mean(rec_df[,8],na.rm=TRUE))
tmp<-apply(rec_df,1,function(x) {which(x==max(x))})
rec[[3]]<-c(find_appearances_list(tmp,value=1),find_appearances_list(tmp,value=2),find_appearances_list(tmp,value=3),find_appearances_list(tmp,value=4),find_appearances_list(tmp,value=5),find_appearances_list(tmp,value=6),find_appearances_list(tmp,value=7),find_appearances_list(tmp,value=8))
names(rec)<-c("df","means","maximum_appearance")

fsc<-list()
fsc[[1]]<-fsc_df
fsc[[2]]<-c(mean(fsc_df[,1],na.rm=TRUE),mean(fsc_df[,2],na.rm=TRUE),mean(fsc_df[,3],na.rm=TRUE),mean(fsc_df[,4],na.rm=TRUE),mean(fsc_df[,5],na.rm=TRUE),mean(fsc_df[,6],na.rm=TRUE),mean(fsc_df[,7],na.rm=TRUE),mean(fsc_df[,8],na.rm=TRUE))
tmp<-apply(fsc_df,1,function(x) {which(x==max(x))})
fsc[[3]]<-c(find_appearances_list(tmp,value=1),find_appearances_list(tmp,value=2),find_appearances_list(tmp,value=3),find_appearances_list(tmp,value=4),find_appearances_list(tmp,value=5),find_appearances_list(tmp,value=6),find_appearances_list(tmp,value=7),find_appearances_list(tmp,value=8))
fsc[[4]]<-c(length(which(is.na(fsc_df[,1]))),length(which(is.na(fsc_df[,2]))),length(which(is.na(fsc_df[,3]))),length(which(is.na(fsc_df[,4]))),length(which(is.na(fsc_df[,5]))),length(which(is.na(fsc_df[,6]))),length(which(is.na(fsc_df[,7]))),length(which(is.na(fsc_df[,8]))))
names(fsc)<-c("df","means","maximum_appearance","non_zero_models")
